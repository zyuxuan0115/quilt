...
fn main() {
  ...
  let in_date = NaiveDate::parse_from_str(&args.in_date[..], "%Y-%m-%d").unwrap();
  let out_date = NaiveDate::parse_from_str(&args.out_date[..], "%Y-%m-%d").unwrap();
  let next_day = in_date.succ_opt().unwrap();

  let mut suffix: String = String::from("_");
  suffix.push_str(&args.in_date[..]);
  suffix.push_str(&args.out_date[..]);
  if out_date > in_date {
    let hotel_dates: Vec<String> = hotel_ids.iter().map(|x| {let mut y = x.clone(); y.push_str(&suffix[..]); y}).collect();
    let hotel_date_strslice: Vec<&str> = hotel_dates.iter().map(|x| &**x).collect();
    let hotel_date_keys: &[&str] = &hotel_date_strslice;
    let result: std::collections::HashMap<String, String> = memcache_client.gets(hotel_date_keys).unwrap();
    let mut reservation_info: Vec<HotelReservation> = Vec::new();
    for (key, value) in result {
      //reservation_not_cached.remove(&key);
      let resv_info: HotelReservation = serde_json::from_str(&value).unwrap();
