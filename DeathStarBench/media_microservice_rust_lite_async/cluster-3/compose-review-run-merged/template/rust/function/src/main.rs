...
fn main() {
  ...
  let handle_unique_id = thread::spawn(move || {
    make_rpc("unique-id-service-merged", uniq_id_args_str)
  });
  ...
  let handle_text = thread::spawn(move || {
    make_rpc("text-service-merged", text_service_arg_str)
  });
  ...
  let handle_user = thread::spawn(move || {
    make_rpc("upload-user-with-username-merged", upload_user_arg_str)
  });
  ...
  let handle_movie = thread::spawn(move ||{
    make_rpc("upload-movie-id-merged", upload_movie_arg_str)
  });
