...
func (bs *BinaryServer) DynamicEntryHandler(w http.ResponseWriter, r *http.Request) {
    path := r.URL.Path
    if path == "/" || path == "/specialize" || path == "/v2/specialize" || path == "/healthz" {
        HttpResponseWithError(w, http.StatusNotFound, fmt.Errorf("route reserved: %s", path))
        return
    }

    functionName := strings.TrimPrefix(path, "/")
    if strings.Contains(functionName, "/") || functionName == "" {
        HttpResponseWithError(w, http.StatusBadRequest, fmt.Errorf("invalid function name: %s", functionName))
        return
    }

    funcPath := filepath.Join("/bin", functionName)
    fileStat, err := os.Stat(funcPath)
    if err != nil || fileStat.IsDir() {
        HttpResponseWithError(w, http.StatusNotFound, fmt.Errorf("function binary not found: %s", funcPath))
        return
    }
    ...
    stdinPipe, err := cmd.StdinPipe()
    if err != nil {
        HttpResponseWithError(w, http.StatusInternalServerError, fmt.Errorf("stdin error: %w", err))
        return
    }

    stdoutPipe, err := cmd.StdoutPipe()
    if err != nil {
        HttpResponseWithError(w, http.StatusInternalServerError, fmt.Errorf("stdout error: %w", err))
        return
    }

    stderrPipe, err := cmd.StderrPipe()
    if err != nil {
        HttpResponseWithError(w, http.StatusInternalServerError, fmt.Errorf("stderr error: %w", err))
        return
    }

    if err := cmd.Start(); err != nil {
        HttpResponseWithError(w, http.StatusInternalServerError, fmt.Errorf("start error: %w", err))
        return
    }
...
func main() {
	...
	httpServer := &http.Server{
        ...
        dynamicMux.HandleFunc("/", server.DynamicEntryHandler)

        dynamicServer := &http.Server{
          Addr:    ":8080",
          Handler: dynamicMux,
        }
	...
	log.Println("Server started")
        go func() {
               err := dynamicServer.ListenAndServe()
               if err != nil && err != http.ErrServerClosed {
                 log.Fatal("DynamicServer ListenAndServe: ", err)
               }
        }()
