...
function build_openfaas {
    ...
    sudo docker build -t zyuxuan0115/hr-$FUNC-async:latest \
...
function build_openwhisk {
    ...
    cp -r $ROOT_DIR/../../DbInterface $ROOT_DIR/template/rust
    sudo docker build --no-cache -t zyuxuan0115/hr-$FUNC-async:latest \
...
function build_fission_bin {
    ...
    rm -rf $ROOT_DIR/template/rust/DbInterface
    sudo docker system prune -f
    sudo docker create --name temp-container zyuxuan0115/hr-$FUNC-async:latest
...
function push {
  sudo docker push zyuxuan0115/hr-$FUNC-async:latest
}
...
function deploy_openwhisk {
  wsk action create $FUNC --docker zyuxuan0115/hr-$FUNC-async:latest    
}
...
#    --image docker.io/zyuxuan0115/hr-$FUNC-async --port 8888 \
