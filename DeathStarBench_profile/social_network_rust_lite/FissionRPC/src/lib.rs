...
static mut AGENT: Option<PyroscopeAgent<PyroscopeAgentRunning>> = None;

fn start_profiling_agent() {
  unsafe {
    // Initialize AGENT if it's not already initialized
    if AGENT.is_none() {
      let agent = PyroscopeAgent::builder("http://pyroscope.pyroscope-test.svc.cluster.local.:4040", "fission-function")
        .backend(pprof_backend(PprofConfig::new().sample_rate(100)))
        .build()
        .unwrap();
      let agent_running = agent.start().unwrap();
      AGENT = Some(agent_running);
    }
  }
}

fn stop_profiling_agent() {
  unsafe {
    if ! AGENT.is_none() {
      if let Some(running) = AGENT.take() {
