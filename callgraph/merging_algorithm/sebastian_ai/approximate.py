...
import collections
import itertools
import random # Needed for generation
...
def select_candidate_roots(graph, root_node, num_candidates):
    """
    Selects potential root candidates based on weighted in-degree.

    Args:
        graph (nx.DiGraph): The input graph.
        root_node: The main root of the graph (always included).
        num_candidates (int): The approximate number of *additional* candidates to select.

    Returns:
        set: A set containing the selected candidate nodes (excluding root_node).
    """
    if num_candidates <= 0:
        return set()
    ...
    for u, v, data in graph.edges(data=True):
        weight = data.get('weight', 1.0) # Default weight if missing
        weighted_in_degrees[v] += weight

    # Sort nodes (excluding the main root) by weighted in-degree, descending
    # items() gives (node, score) pairs
    sorted_nodes = sorted(
        [(node, score) for node, score in weighted_in_degrees.items() if node != root_node],
        key=lambda item: item[1],
        reverse=True
    )

    # Select the top 'num_candidates'
    # Ensure we don't try to select more candidates than available
    actual_num_to_select = min(num_candidates, len(sorted_nodes))
    candidates = {node for node, score in sorted_nodes[:actual_num_to_select]}

    print(f"Selected {len(candidates)} additional root candidates based on weighted in-degree.")
    # print(f"Candidates: {candidates}") # Optional debug print
    return candidates


# --- Root Selection Heuristic (Phase 1) - MODIFIED with Candidate Selection ---
def simple_root_selection_strategy(graph, M, C, root_node, all_nodes, predecessors, full_reachable_from,
                                   max_k=3,
                                   num_root_candidates=20, # New parameter
                                   ilp_time_limit=60,
                                   ilp_log_to_console=False):
    """
    Root selection heuristic using a limited pool of candidates based on heuristics.
    Uses the ILP solver with cross-rule constraint.

    Args:
        graph, M, C, root_node, all_nodes, predecessors, full_reachable_from: Graph info.
        max_k (int): Max number of roots to consider.
        num_root_candidates (int): How many additional nodes (besides root_node) to consider
                                   as potential roots based on heuristics.
        ilp_time_limit (float): Time limit for each ILP solve.
        ilp_log_to_console (bool): Verbosity for Gurobi.

    Returns:
        tuple: (best_cost, best_R, best_assignment) or (None, None, None).
    """
    best_cost = float('inf'); best_R = None; best_assignment = None; tried_R_configs = set()
    print("\n=== Starting Root Selection Heuristic (Candidate Pool Version) ===")
    print(f"Max k: {max_k}, Num Root Candidates (Pool Size): {num_root_candidates}")

    # --- Select the pool of potential additional roots ---
    additional_candidate_pool = select_candidate_roots(graph, root_node, num_root_candidates)
    # The full pool always conceptually includes root_node for k=1
    # For k > 1, combinations are made from additional_candidate_pool
    ...
    for k in range(1, max_k + 1):
        ...
        if k == 1:
            # Only one option: the main root itself
            if root_node in graph: candidate_R_tuples = [(root_node,)]
            else: candidate_R_tuples = []
        else:
            # Choose k-1 additional roots FROM THE CANDIDATE POOL
            if root_node not in graph: candidate_R_tuples = []
            else:
                # Ensure we only try combinations using available candidates
                available_additional_candidates = list(additional_candidate_pool)
                if len(available_additional_candidates) < k - 1:
                     print(f"Warning: Not enough candidates ({len(available_additional_candidates)}) in pool to form combinations for k={k}. Skipping.")
                     candidate_R_tuples = []
                else:
                    combos = itertools.combinations(available_additional_candidates, k - 1)
                    candidate_R_tuples = [(root_node,) + combo for combo in combos]

        if not candidate_R_tuples:
            print(f"No valid candidate root sets to try for k={k}.")
            continue

        print(f"Trying {len(candidate_R_tuples)} combinations for k={k} (from candidate pool)...")
        for r_tuple in candidate_R_tuples:
            # Validation: Ensure all roots in tuple are actually in the graph
            current_R_set_list = [r for r in r_tuple if r in graph]
            if len(current_R_set_list) != len(r_tuple): continue # Should not happen if pool is valid

            current_R_set = frozenset(current_R_set_list)
            if current_R_set in tried_R_configs: continue
            tried_R_configs.add(current_R_set)

            # --- Call the final ILP solver ---
...
def run_test_case(name, graph, M, C,
                  max_k=3,
                  num_root_candidates=20, # Add parameter
                  ilp_time_limit=60,
                  ilp_log_to_console=False):
    """Runs the full process for a given test case."""
    print(f"\n{'='*20} Running Test Case: {name} {'='*20}")
    if len(graph) < 30: print(f"Nodes: {graph.nodes(data=True)}\nEdges: {graph.edges(data=True)}")
    else: print(f"Nodes: {len(graph)}, Edges: {len(graph.edges)}")
    print(f"Constraints: M={M}, C={C}")
    ...
    # --- Call the MODIFIED Root Selection Strategy ---
    cost, R, assign = simple_root_selection_strategy(
        graph, M, C, root, nodes, preds, reach,
        max_k=max_k,
        num_root_candidates=num_root_candidates, # Pass the new parameter
        ilp_time_limit=ilp_time_limit,
        ilp_log_to_console=ilp_log_to_console
    )
    # --- ---
...
# --- Function to Generate Large Test Case ---
def generate_large_rdag(num_nodes, extra_edge_factor=1.0):
    """
    Generates a connected random Directed Acyclic Graph (rDAG) with a single root (node 0).

    Args:
        num_nodes (int): The desired number of nodes.
        extra_edge_factor (float): Multiplier for adding extra non-tree edges.
                                   e.g., 1.0 adds roughly num_nodes extra edges.

    Returns:
        nx.DiGraph: The generated graph with node/edge attributes.
    """
    if num_nodes <= 0:
        return nx.DiGraph()

    G = nx.DiGraph(name=f"LargeRandomRDAG_{num_nodes}")
    G.add_nodes_from(range(num_nodes))

    # 1. Build a random spanning tree rooted at 0 to ensure connectivity + DAG base
    print(f"Generating base tree for {num_nodes} nodes...")
    parents = {} # Keep track for depth calculation later
    for i in range(1, num_nodes):
        # Connect node i to a random node j already in the graph (0 to i-1)
        parent = random.randint(0, i - 1)
        G.add_edge(parent, i)
        parents[i] = parent

    # 2. Calculate depths based on the tree structure (BFS from root 0)
    ...
    while queue:
        u = queue.popleft()
        # Check successors in the *generated* graph so far (which is the tree)
        for v in G.successors(u): # Using G.successors should be correct here
             if v not in visited:
                 visited.add(v)
                 depths[v] = depths[u] + 1
    ...
    # Ensure all nodes got a depth (should happen if tree is connected)
    if len(depths) != num_nodes:
        print(f"Warning: Not all nodes have calculated depths ({len(depths)} vs {num_nodes}). Check connectivity.")
        # Fallback for any missed nodes - assign max depth? Or find root cause.
        for i in range(num_nodes):
             if i not in depths:
                 print(f"Node {i} missed depth calculation.")
                 # Try finding path? This indicates an issue in tree generation or BFS
                 depths[i] = max_depth + 1 # Assign arbitrary large depth


    # 3. Add extra "forward" edges to maintain DAG property
    num_extra_edges = int(num_nodes * extra_edge_factor)
    print(f"Adding approximately {num_extra_edges} extra forward edges...")
    added_edges = 0
    attempts = 0
    max_attempts = num_extra_edges * 5 # Limit attempts to avoid infinite loops
    ...
    while added_edges < num_extra_edges and attempts < max_attempts:
        ...
        # Ensure u, v have depths, are different, edge doesn't exist, and is forward
        if u != v and u in depths and v in depths and depths[u] < depths[v] and not G.has_edge(u, v):
            G.add_edge(u, v)
            added_edges += 1

    print(f"Added {added_edges} extra edges after {attempts} attempts.")
...
if __name__ == "__main__":
    ...
    run_test_case("Force Cross Edge", G5, M5, C5, max_k=4) # Expect R={0,1,2,3}, Cost=30
      ...
      # --- Large Test Case ---
    NUM_NODES_LARGE = 20
    EXTRA_EDGE_FACTOR_LARGE = 1.2 # Add slightly more edges than nodes

    print(f"\nGenerating large test graph ({NUM_NODES_LARGE} nodes)...")
    G_large = generate_large_rdag(NUM_NODES_LARGE, EXTRA_EDGE_FACTOR_LARGE)

    # Calculate reasonable M and C based on totals
    total_m = sum(d.get('m', 0) for _, d in G_large.nodes(data=True))
    total_c = sum(d.get('c', 0) for _, d in G_large.nodes(data=True))
    print(f"Total m: {total_m}, Total c: {total_c}")

    # Aim for roughly 5-10 subgraphs maybe? Adjust divisor as needed.
    divisor = 6
    M_large = 300
    C_large = 300
    print(f"Setting M = {M_large}, C = {C_large}")

    # Set parameters for the large run
    # max_k might need to be higher if M/C are small relative to totals
    MAX_K_LARGE = 9 # Allow exploring more roots
    TIME_LIMIT_LARGE = 60 # seconds (5 minutes) - adjust as needed
    NUM_CANDIDATES_LARGE = 10 # Limit the pool of additional roots (tune this!)

    run_test_case(f"Large Random ({NUM_NODES_LARGE} nodes)",
                  G_large,
                  M_large, C_large,
                  max_k=MAX_K_LARGE,
                  num_root_candidates=NUM_CANDIDATES_LARGE, # Use the limited pool
                  ilp_time_limit=TIME_LIMIT_LARGE,
                  ilp_log_to_console=False) # Keep console clean unless debugging ILP
