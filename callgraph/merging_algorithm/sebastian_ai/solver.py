...
# --- Main Execution Block (run_test_case - Keep As Is) ---
def run_test_case(name, graph, M, C, max_k=3, ilp_time_limit=60, ilp_log_to_console=False):
    ...
    # Limit printing for large graphs
    if len(graph) < 30:
    ...
    # --- Call the final Root Selection Strategy ---
    cost, R, assign = simple_root_selection_strategy(
        graph, M, C, root, nodes, preds, reach,
        max_k=max_k, ilp_time_limit=ilp_time_limit, ilp_log_to_console=ilp_log_to_console
    )
    # --- ---

    print(f"\nExecution time {name}: {time.time() - start:.2f}s")
    if assign:
        print_solution_details(graph, M, C, R, assign)
        print(f"Final Reported Cost (ILP): {cost:.4f}")
    else:
...
if __name__ == "__main__":
    ...
    TIME_LIMIT_LARGE = 300 # seconds (5 minutes) - adjust as needed
