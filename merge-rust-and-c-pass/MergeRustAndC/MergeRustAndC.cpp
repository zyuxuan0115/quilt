...
*/
  ...
  bool MergeRustAndC::isRPC(Instruction* Inst){
...
//	}
  ...
  StringRef MergeRustAndC::getRPCCalleeName(Instruction* Inst){
    StringRef CalleeName = "";
    if ((isa<CallInst>(Inst)) && (dyn_cast<CallInst>(Inst)->isIndirectCall())){
      CallInst* VirtualCall = dyn_cast<CallInst>(Inst);
      if (VirtualCall->getNumOperands()==7){
...
static RegisterPass<MergeRustAndC> X("MergeRustAndC", "Convert RPC to normal function call by changing the arguments of the function and the call instruction");
