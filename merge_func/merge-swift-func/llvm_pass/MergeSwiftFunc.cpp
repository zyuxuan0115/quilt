//===--------- MergeSwiftFunc.cpp - Transformations --------------------------===//
//
...
#include "llvm/Transforms/Utils/MergeSwiftFunc.h"
#include <unistd.h>
#include <sys/wait.h>

using namespace llvm;

static cl::opt<bool> RenameCallee_ss(
                                     "rename-callee-ss", cl::init(false),
                                     cl::desc("rename the wrapper functions"));

static cl::opt<bool> RenameWrapper_ss(
                                     "rename-wrapper-ss", cl::init(false),
                                     cl::desc("rename the wrapper functions"));

static cl::opt<bool> MergeCallee_ss(
                                     "merge-callee-ss", cl::init(false),
                                     cl::desc("merge the given callee functions"));


PreservedAnalyses MergeSwiftFuncPass::run(Module &M,
                                       ModuleAnalysisManager &AM) {
  if (RenameCallee_ss) {
    RenameCallee(&M);
  }
  else if (RenameWrapper_ss) {
    RenameWrapper(&M);
  }
  else if (MergeCallee_ss) {
    MergeCallee(&M);
  }
  return PreservedAnalyses::all();
}

void MergeSwiftFuncPass::RenameCallee(Module* M) {
  Function *mainFunc = M->getFunction("main");
  mainFunc->setName("main_callee");  
}

void MergeSwiftFuncPass::RenameWrapper(Module* M) {
  Function *mainFunc = M->getFunction("main");
  mainFunc->setName("main_wrapper");  
}

void MergeSwiftFuncPass::MergeCallee(Module* M) {
...
std::string MergeSwiftFuncPass::getDemangledFunctionName(std::string mangledName) {
...
Function* MergeSwiftFuncPass::getFunctionByDemangledName(Module* M, std::string fname) {
...
CallInst* MergeSwiftFuncPass::getCallInstByCalledFunc(Function* callerFunc, Function* calledFunc) {
...
CallInst* MergeSwiftFuncPass::createCallWrapper(CallInst* rpcInst, Function* wrapperFunc) {
...
Function* MergeSwiftFuncPass::createNewCalleeFunc(Function* calleeFunc, CallInst* dummyCall) {
...
void MergeSwiftFuncPass::createCall2NewCallee(CallInst* dummyCall, Function* newCalleeFunc) {
  std::vector<Value*> arguments;
  std::vector<Type*> argumentTypes;
  for (unsigned i=0; i<dummyCall->getNumOperands()-1; i++){
    Value* arg = dummyCall->getOperand(i);
