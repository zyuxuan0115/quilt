...
bars2 = ax.bar(x - width/2, [0,0,0], width, label='50% latency async')
bars3 = ax.bar(x + width/2, ninety_latency_normalized, width, label='99% latency sync')
bars4 = ax.bar(x + width*1.5, [0,0,0], width, label='99% latency async')

label = ["search-handler(10)", "reservation-handler(3)", "nearby-cinema(2)"]
