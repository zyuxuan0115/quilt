...
request = function(req_id)
  ...
  local method = "POST"
  local path = "/api/v1/namespaces/_/actions/social-graph-follow?blocking=true&result=true"
  local headers = {}
  local body
  headers["Content-Type"] = "application/json"
  headers["Authorization"] = auth

  body = '{"user_id":' .. tostring(user_index) .. ',"followee_id":' .. tostring(followee_idx) .. '}'

  local body_write = body .. '\n'
  file = io.open('req_data_log_social-graph-follow.txt', 'a')
